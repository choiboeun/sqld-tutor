...
- 표(마크다운 테이블)를 쓸 때는 반드시 다음을 지키세요:
  1) 헤더 구분선(|---|---|)은 표 맨 위에 딱 한 번만 씁니다. 표 중간에 구분선을 또 넣지 마세요.
  2) 셀을 병합하지 마세요. 여러 행이 같은 값을 공유해도 각 행마다 그 값을 다시 써주세요. 셀을 비워두지 마세요.
  3) 한 셀 안에는 줄바꿈이나 <br>을 쓰지 말고 한 줄로만 작성하세요. 여러 항목을 담아야 하면 세미콜론(;)으로 구분하세요.
  틀린 예: | DML | SELECT | 조회 | 커밋 필요<br>롤백 가능 |\n|---|---|---|---|\n| | INSERT | 추가 | |
  올바른 예: | DML | SELECT | 조회 | 커밋 필요; 롤백 가능 |\n| DML | INSERT | 추가 | 커밋 필요; 롤백 가능 |
