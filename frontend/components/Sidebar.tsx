...
export default function Sidebar({ threadId, refresh, liveStats, onStatsRefreshed, isOpen = false, onClose, highlightHome = false }: Props) {
  ...
  const asideClass = "flex flex-col bg-violet-50 border-r border-violet-100 p-5 gap-5 overflow-y-auto";
  ...
  return (
    <>
      ...
      <aside className={
        ...
        {/* Logo + 홈으로 */}
        <div className="pb-4 border-b border-violet-100">
          <Link
            href="/home"
            onClick={onClose}
          ...
          >
            ← 홈으로
            {highlightHome && <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse ml-0.5" />}
          </Link>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-indigo-500 flex items-center justify-center text-xs font-black text-white shrink-0">
                ◇
              </div>
              <span className="text-sm font-bold text-indigo-900">SQLD AI 튜터</span>
            </div>
            <button
              onClick={onClose}
              className="md:hidden text-indigo-300 hover:text-indigo-500 transition-colors p-1"
              aria-label="닫기"
            >
        ...
        {/* 학습 현황 */}
        <div>
          <h2 className="text-xs font-semibold tracking-widest text-indigo-300 uppercase mb-3">학습 현황</h2>

          {/* 예상 점수 */}
          <div className="bg-white px-4 py-3 mb-3 border border-violet-100">
            <div className="flex items-baseline justify-between mb-1.5">
              <span className="text-xs text-indigo-400 font-semibold">예상 점수</span>
              <span className="text-xs font-semibold text-indigo-500">
                목표 {targetScore}점 {scoreDiff >= 0 ? `+${scoreDiff}` : scoreDiff}점
              </span>
            </div>
            <div className="flex items-end gap-1.5">
              <span className="text-3xl font-bold text-indigo-950 leading-none">{predictedScore}</span>
              <span className="text-sm text-indigo-300 mb-0.5">/ 100점</span>
            </div>
            ...
            {totalAnswered === 0 && (
              <p className="text-xs text-indigo-300 mt-1.5">문제를 풀면 점수가 계산됩니다</p>
            )}
          </div>

          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="bg-white px-3 py-2 text-center border border-violet-100">
              <p className="text-lg font-bold text-indigo-900">{totalAnswered}문제</p>
              <p className="text-xs text-indigo-300 mt-0.5">풀이 수</p>
            </div>
            <div className="bg-white px-3 py-2 text-center border border-violet-100">
              <p className={`text-lg font-bold ${streak > 0 ? "text-indigo-500" : "text-indigo-900"}`}>{streak}개</p>
              <p className="text-xs text-indigo-300 mt-0.5">연속 정답</p>
            </div>
          </div>

          {/* 전체 진행률 */}
          <div className="bg-white px-3 py-2 border border-violet-100">
        ...
        {/* 카테고리별 정답률 */}
        <div>
          <h3 className="text-xs font-semibold text-indigo-300 uppercase tracking-widest mb-2">
            카테고리별 정답률
          </h3>
          <div className="space-y-3">
            {sortedCats.map((cat) => {
              ...
              const dotColor = !tried ? "bg-violet-200"
                : acc === 0 ? "bg-red-300"
                : acc < 0.4 ? "bg-rose-400"
              ...
              return (
                <div key={cat} title={cat}>
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className={`truncate text-xs font-medium flex-1 ${tried ? "text-indigo-700" : "text-indigo-300"}`}>
                      {shortName(cat)}
                    </span>
                    <span className={`text-xs shrink-0 tabular-nums ${tried ? "text-indigo-500 font-semibold" : "text-indigo-200"}`}>
            ...
            <ul className="space-y-1.5">
              {weakCategories.map(({ category, accuracy }) => (
                <li key={category} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="truncate text-indigo-700" title={category}>{shortName(category)}</span>
                  <span className="ml-auto shrink-0 font-semibold text-rose-500">{Math.round(accuracy * 100)}%</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <Link
          href="/wrong-answers"
          className="flex items-center justify-between w-full px-4 py-2.5 bg-white hover:bg-violet-50 transition-colors text-indigo-700 border border-violet-100 hover:border-indigo-200 hover:text-indigo-600"
          onClick={onClose}
        >
