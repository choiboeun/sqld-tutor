    ...
    <div className="min-h-screen flex items-center justify-center bg-stone-50">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
        <div className="text-center mb-7">
          ...
          <h1 className="text-xl font-bold text-stone-900 mb-1">목표 점수를 알려주세요</h1>
          <p className="text-sm text-stone-500">SQLD 시험 목표에 맞춰 학습을 조정할게요</p>
        </div>
        ...
        <div className="space-y-2 mb-8">
          {SCORE_OPTIONS.map((opt) => (
            ...
            >
              ...
              <span className={targetScore === opt.value ? "text-amber-600 text-xs" : "text-stone-400 text-xs"}>
                {opt.desc}
              </span>
            </button>
          ))}
        ...
        <button
          onClick={handleStart}
          disabled={loading}
          className="w-full bg-amber-600 text-white py-3 rounded-xl text-sm font-semibold hover:bg-amber-700 disabled:opacity-40 transition-colors"
        >
